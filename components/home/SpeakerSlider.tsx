...
function Arrow({ type, onClick, isEdge }) {
  const pointer =
    type === "PREV" ? (
      <FontAwesomeIcon
        icon={faAngleLeft}
        className={`${isEdge ? "text-gray-400 cursor-not-allowed" : "cursor-pointer text-white"} block h-8 w-8`}
      />
    ) : (
      <FontAwesomeIcon
        icon={faAngleRight}
        className={`${isEdge ? "text-gray-400 cursor-not-allowed" : "cursor-pointer text-white"} block h-8 w-6`}
      />
    );
  return (
